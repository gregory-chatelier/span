...
#
# Usage:
#   curl -sSfL https://raw.githubusercontent.com/gregory-chatelier/span/install.sh | sh
#
# To specify a custom installation directory, set the INSTALL_DIR environment variable
# for the 'sh' command:
#   curl -sSfL https://raw.githubusercontent.com/gregory-chatelier/span/install.sh | INSTALL_DIR=~/my-bin sh
...
if [ "$INSTALL_DIR" = "/usr/local/bin" ] && [ "$(id -u)" -ne 0 ]; then
    ...
    exec sudo sh -c "$(cat "$0" 2>/dev/null || curl -sSfL https://raw.githubusercontent.com/gregory-chatelier/span/main/install.sh)" "$@"
fi
