...
func Deval(val, a, b float64) (float64, error) {
	delta := b - a
	if delta == 0 {
		// If the value is at the zero-delta point, t is 0. Otherwise, it's undefined.
		if val == a {
			return 0, nil
		}
		return 0, fmt.Errorf("cannot de-evaluate in an interval with zero delta")
	}
	return (val - a) / delta, nil
}
...
func Snap(val float64, steps int, a, b float64) (float64, error) {
	...
	t, _ := Deval(val, a, b) // We know delta is not zero here
...
func Random(r *rand.Rand, count int, a, b float64) ([]float64, error) {
	...
	for i := 0; i < count; i++ {
		t := r.Float64() // A random float64 in [0.0, 1.0)
		results[i] = Eval(t, a, b)
	}
